import numpy as np
from autodiff32 import AutoDiffObj as ad
...
def logistic(x):
    try:
        return ad.AutoDiff(np.exp(x.val)/(np.exp(x.val) + 1), np.dot(np.exp(x.val)/((np.exp(x.val) + 1)**2),x.der))
    except Exception:
        return (np.exp(x)/(np.exp(x) + 1))

def log(x, base=np.e): # accepts any value for the base, but default is natural log
    try:
        return ad.AutoDiff(np.log(x.val)/np.log(base), (1/(np.log(base) * x.val)) * x.der)
    except Exception:
...
def sin(x):
    try:
        return ad.AutoDiff(np.sin(x.val), np.cos(x.val) * x.der)
    except Exception:
        return np.sin(x)
...
def sinh(x):
    try:
        return ad.AutoDiff(np.sinh(x.val), x.der*np.cosh(x.val))
    except Exception:
        return np.sinh(x)
